import { QueryClient, QueryClientProvider } from '@tanstack/react-query'
import { RouterProvider } from '@tanstack/react-router'
import { render, screen } from '@testing-library/react'
import { http, HttpResponse } from 'msw'
import type { ReactNode } from 'react'
import { describe, it, expect } from 'vitest'

import { server } from '../../mocks/server'
import { createTestRouter } from '../../test/createTestRouter'

function wrapper({ children }: { children: ReactNode }) {
  const client = new QueryClient({
    defaultOptions: { queries: { retry: false } },
  })
  return <QueryClientProvider client={client}>{children}</QueryClientProvider>
}

describe('InsightsPage', () => {
  it('renders both charts after data loads', async () => {
    server.use(
      http.get('*/insights/by-country', () =>
        HttpResponse.json([
          {
            country: 'US',
            headcount: 1000,
            min_salary_cents: 5_000_000,
            max_salary_cents: 30_000_000,
            avg_salary_cents: 12_000_000,
            median_salary_cents: 11_000_000,
            p25_salary_cents: 9_000_000,
            p75_salary_cents: 15_000_000,
          },
        ]),
      ),
      http.get('*/insights/by-country-job-title', () =>
        HttpResponse.json([
          {
            country: 'US',
            job_title: 'Engineer',
            headcount: 50,
            avg_salary_cents: 13_000_000,
          },
        ]),
      ),
    )

    const testRouter = createTestRouter(['/insights'])
    render(<RouterProvider router={testRouter} />, { wrapper })

    await screen.findByText('Salary insights')
    expect(screen.getByText('Salary distribution by country')).toBeInTheDocument()
    expect(
      screen.getByText('Top 15 average salary by country and job title'),
    ).toBeInTheDocument()
  })
})
